...
import { Task, Note, TaskService, NoteService, LocalTaskService, LocalNoteService } from "@/lib/database"
...
export function UnifiedSearch({ user, onTaskSelect, onNoteSelect }: UnifiedSearchProps) {
  ...
  const performSearch = async (term: string) => {
    ...
    try {
      ...
      if (isGuestMode) {
        ...
        notes.forEach(note => {
          const relevance = calculateRelevance(term, note.title, note.content)
  ...
  const calculateRelevance = (searchTerm: string, title: string, content: string): number => {
  ...
  const handleItemClick = (result: SearchResult) => {
    if (result.type === 'task' && onTaskSelect) {
      ...
      onNoteSelect(result.item as Note)
    }
  }
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          <Badge variant={result.type === 'task' ? 'default' : 'secondary'} className="text-xs">
                          ...
                          {result.type === 'note' && (result.item as Note).is_pinned && (
                            <Badge variant="outline" className="text-xs">
                              📌 Pinned
                            </Badge>
                          )}
                        ...
                        <p className="text-xs text-muted-foreground line-clamp-2 mt-1 capitalize">
                          {result.type === 'task' 
                            ? (result.item as Task).description || 'No description'
                        ...
                        <div className="flex items-center gap-2 mt-2">
                          ...
                          {result.type === 'task' && (result.item as Task).category && (
                            <Badge variant="outline" className="text-xs">
                              {(result.item as Task).category}
                            </Badge>
                          )}
                          
                          {result.type === 'note' && (result.item as Note).tags && (result.item as Note).tags!.length > 0 && (
                            <div className="flex gap-1">
                              {(result.item as Note).tags!.slice(0, 2).map((tag, tagIndex) => (
